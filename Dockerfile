...
RUN apt-get -qq update && apt-get -y upgrade && apt-get -y --no-install-recommends install gcc-8 ninja-build locales python3-pip python3-setuptools python3-wheel valgrind aria2 libc6:i386 libstdc++6:i386 zlib1g:i386 libmpc3:i386 xz-utils git make qemu-user-static ccache wget curl libncurses5-dev pkg-config gnupg python3-venv mingw-w64 autoconf automake libtool osslsigncode p7zip-full libcurl4-gnutls-dev strace git-lfs byacc flex
RUN git-lfs install

...
RUN wget -qO- https://apt.llvm.org/llvm-snapshot.gpg.key | apt-key add - && printf "deb http://apt.llvm.org/bionic/ llvm-toolchain-bionic-8 main\ndeb http://apt.llvm.org/bionic/ llvm-toolchain-bionic-9 main\ndeb http://apt.llvm.org/bionic/ llvm-toolchain-bionic main" > /etc/apt/sources.list.d/llvm.list && apt-get -qq update && apt-get -y --no-install-recommends install clang-8 llvm-8-dev clang-9 llvm-9-dev clang-10 llvm-10-dev && ln -s /usr/bin/clang-9 /usr/local/bin/clang
RUN apt-get autoremove --purge && apt-get autoclean
RUN locale-gen --lang en_US.UTF-8 && pip3 install meson awscli && python3 -m venv /opt/meson-0.46.1 && . /opt/meson-0.46.1/bin/activate && pip3 install wheel && pip3 install meson==0.46.1 && printf "#!/bin/sh -e\n. /opt/meson-0.46.1/bin/activate\nexec meson \"\$@\"" > /usr/local/bin/meson-0.46.1 && chmod 755 /usr/local/bin/meson-0.46.1

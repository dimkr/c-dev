...
RUN locale-gen --lang en_US.UTF-8 && python3 -m pip install --no-use-pep517 meson && python3 -m pip install awscli gcovr git-filter-repo && for v in 0.46.1 0.51.2; do python3 -m venv /opt/meson-$v && . /opt/meson-$v/bin/activate && python3 -m pip install meson==$v && printf "#!/bin/sh -e\n. /opt/meson-$v/bin/activate\nexec meson \"\$@\"" > /usr/local/bin/meson-$v && chmod 755 /usr/local/bin/meson-$v; done

...
RUN dpkg --add-architecture i386 && sed -i s/^deb.*multiverse.*/\#\&/g /etc/apt/sources.list && apt-get -qq update && apt-get -y upgrade && apt-get -y --no-install-recommends install gcc ninja-build python3-pip python3-setuptools python3-wheel python3-requests valgrind libc6:i386 libstdc++6:i386 zlib1g:i386 libmpc3:i386 xz-utils git make qemu-user-static ccache libncurses5-dev python3-venv strace git-lfs byacc flex wget gnupg autoconf automake libtool afl gdb-multiarch && git-lfs install

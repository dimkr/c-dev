...
RUN apt-get -qq update && apt-get -y --no-install-recommends install gcc gcc-8 clang llvm-dev ninja-build locales python3-pip python3-setuptools python3-wheel valgrind aria2 libc6:i386 libstdc++6:i386 zlib1g:i386 libmpc3:i386 xz-utils git make qemu-user-static ccache wget curl libncurses5-dev pkg-config && apt-get autoremove --purge && apt-get autoclean

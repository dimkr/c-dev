...
RUN locale-gen --lang en_US.UTF-8 && for v in 0.46.1 0.51.2; do pip3 install meson awscli && python3 -m venv /opt/meson-$v && . /opt/meson-$v/bin/activate && pip3 install wheel && pip3 install meson==$v && printf "#!/bin/sh -e\n. /opt/meson-$v/bin/activate\nexec meson \"\$@\"" > /usr/local/bin/meson-$v && chmod 755 /usr/local/bin/meson-$v; done
